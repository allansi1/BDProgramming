package TotalizationCalculatorAnnee;

import java.sql.*;
import java.util.Calendar;

public class TotalizationCalculatorAnnee {
    ...
    public static void main(String[] args) {
        try (Connection connection = DriverManager.getConnection(JDBC_URL, JDBC_USER, JDBC_PASSWORD)) {
            TotalizationCalculatorAnnee calculator = new TotalizationCalculatorAnnee();
            calculator.calculateTotalizations(connection);
            System.out.println("Totalizations completed!");
        } catch (SQLException ex) {
            ex.printStackTrace();
            System.err.println("Error to connect do BD.");
        }
    }

    public void calculateTotalizations(Connection connection) throws SQLException {
        String selectPeriodeQuery = "SELECT * FROM Periode WHERE TypePeriode = 'Anuelle'";
