...
//	}
	...
	class MyEditTableModel extends DefaultTableModel {
		private String columnNames []={"ID", "NAME", "AGE", "ADRESS",
		"SALARY"};
...
//	}
		...
		btnOk.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(isUpdate) {					
					if (myValidate()) {
						try {
							String query = "UPDATE company SET name = '"+ table.getValueAt(0, 1)+"',"+
									" age = "+table.getValueAt(0, 2)+","+
									" adress =  '"+ table.getValueAt(0, 3) +"',"+
									" salary = "+ table.getValueAt(0, 4) +
									" WHERE id = "+table.getValueAt(0, 0);
							
							// Prepare Statement (Pooh)
							
							EmpGUI.current.stmt.executeUpdate(query);
					...
					if (myValidate()) {
						try {
							...
							String query = "INSERT INTO company (id,name,age,adress,salary)" +
									" VALUES (" +
							...
							// Se nada der certo, valida se tem apóstrofe (') e -- e isso deve resolver
	...
	private boolean myValidate() {	
		...
		if (!isFirstCharUppercase(table.getValueAt(0, 1))) {
			JOptionPane.showMessageDialog(null, "Name invalide");
