...
import etudiant.EditForm.MyEditTableModel;
		...
		btnOk.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(isUpdate) {					
					if (myValidate()) {
						try {
							EmpGUI.current.stmt.executeUpdate(
									"UPDATE etudiant SET E_Pnom = '"+ table.getValueAt(0, 1)+"',"+
									" E_nom = '"+table.getValueAt(0, 2)+"',"+
					...
					if (myValidate()) {
						try {
							EmpGUI.current.stmt.executeUpdate(
									"INSERT INTO etudiant (E_id,E_Pnom, E_nom)" +
									" VALUES (" + "'"+ table.getValueAt(0, 0) + "',"+
		...
		if(!isUpdate) {      // si c'est une insertion
			...
			try {
				ResultSet rs = EmpGUI.current.stmt.executeQuery( 
						"SELECT * FROM etudiant WHERE id =  "+ table.getValueAt(0, 0)
						);
				if (rs.first()) {
