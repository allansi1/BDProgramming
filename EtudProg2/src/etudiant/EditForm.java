...
//	}
	...
	class MyEditTableModel extends DefaultTableModel {
		private String columnNames []={"E_ID", "E_PNom", "E_Nom"};
		...
		btnOk.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (isUpdate) {
				    if (myValidate()) {
				        try {
				        	String updateQuery = "UPDATE etudiant SET E_Pnom = ?, E_nom = ? WHERE E_Id = ?";
				            PreparedStatement pstmt = EmpGUI.current.con.prepareStatement(updateQuery);
				            pstmt.setString(1, (String) table.getValueAt(0, 1));
				            pstmt.setString(2, (String) table.getValueAt(0, 2));
				        ...
				        EditForm.current.dispose(); // pour fermer la fenêtre.
				    } // end if validate()
				} else { // not isUpdate -- add
				    if (myValidate()) {
				        try {
				        	 String insertQuery = "INSERT INTO etudiant (E_Id, E_Pnom, E_nom) VALUES (?, ?, ?)";
				             PreparedStatement pstmt = EmpGUI.current.con.prepareStatement(insertQuery);
				             pstmt.setString(1, (String) table.getValueAt(0, 0));
				             pstmt.setString(2, (String) table.getValueAt(0, 1));
				             pstmt.setString(3, (String) table.getValueAt(0, 2));
				             pstmt.executeUpdate();
		...
		if(!isUpdate) {      // si c'est une insertion
			...
			try {
				ResultSet rs = EmpGUI.current.stmt.executeQuery( 
						"SELECT * FROM etudiant WHERE E_Id =  '"+ table.getValueAt(0, 0)+"'"
						);
				if (rs.first()) {
