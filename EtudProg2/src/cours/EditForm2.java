...
public class EditForm2 extends JDialog {
	...
	public static EditForm2 current;    /* pour sauter d'une fen�tre � l'autre */
...
//	}
	...
	class MyEditTableModel extends DefaultTableModel {
		private String columnNames []={"C_ID", "C_nom", "C_Duree"};
...
//	}
	...
	public EditForm2() {
				...
				if (isUpdate) {
				    if (myValidate()) {
				        try {
				            ...
				            PreparedStatement pstmt = EmpGUI2.current.con.prepareStatement(updateQuery);
				        ...
				        EditForm2.current.dispose(); // pour fermer la fenêtre.
				    } // end if validate()
				} else { // not isUpdate -- add
				    if (myValidate()) {
				        try {
				        	 String insertQuery = "INSERT INTO cours (C_Id, C_nom, C_duree) VALUES (?, ?, ?)";
				             PreparedStatement pstmt = EmpGUI2.current.con.prepareStatement(insertQuery);
		...
		btnCancel.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				EditForm2.current.dispose();   // pour fermer la fen�tre. 
			}
		});
	...
	private boolean myValidate() {	
		...
		if(!isUpdate) {      // si c'est une insertion
			if(!isValidId(table.getValueAt(0, 0))) {    // si le nouveau id est invalse (n�gatif)
			 ...
			 */
			try {
				ResultSet rs = EmpGUI2.current.stmt.executeQuery( 
						"SELECT * FROM cours WHERE C_Id =  '"+ table.getValueAt(0, 0)+"'"
						);
				if (rs.first()) {
					JOptionPane.showMessageDialog(null, "Id répété: id existe d�j� dans la base de données");
					return false;                            // dans ce cas, la fonction fini ici 
				}
	...
	static boolean isValidId(Object eId) {
		if (eId==null){return false; }
		try{
			return ((String) eId).matches("^C[0-9]{5}$");
		}
		catch (Exception e){
			return false;
		}
